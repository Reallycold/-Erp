...
public class OrdersBiz extends BaseBiz<Orders> implements IOrdersBiz {
	...
	public void add(Orders orders){
		...
		for (Orderdetail detail : orders.getOrderDetails()) {
			...
			//设置明细对应的订单 原因 ： orders采用级联跟新 且外键交给明细来维护
	...
	public void doCheck(Long uuid, Long empUuid) {
		...
		if(!Orders.STATE_CREATE.equals(orders.getState())){
			throw new ErpException("亲订单已经审核过了");
	...
	public void exportById(OutputStream os, Long uuid) {
		...
		Map<Long, String> empNameMap = new HashMap<Long, String>();		
		//设置经办人
		sheet.getRow(3).getCell(3).setCellValue(getEmpName(orders.getCreater(),empNameMap));
		sheet.getRow(4).getCell(3).setCellValue(getEmpName(orders.getChecker(),empNameMap));
		sheet.getRow(5).getCell(3).setCellValue(getEmpName(orders.getStarter(),empNameMap));
		sheet.getRow(6).getCell(3).setCellValue(getEmpName(orders.getEnder(),empNameMap));
